...
function Project() {
  ...
  function removeService( id, cost) {
    ...
    const servicesUpdated = project.sevices.filter(
    ...
    fetch('htto://localhost:5000/project/${projectUpdated.id}', {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringofy(projectUpdated)
    })
    .then((resp) => resp.json())
    .then((data) => {
        ...
        <div className={styles.project_details}>
          <Container customClass="column">
            ...
            <Container customClass="start">
              ...
              {services.length === 0 && <p>Nao há servicos cadastrados.</p>}
            </Container>
          </Container>
        </div>
      ) : (
